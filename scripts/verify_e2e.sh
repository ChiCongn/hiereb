...
#   DATASET_PRESET=custom DATA_FILE=all-house/one-day.csv AUTO_DETECT_HOUSE_IDS=1 bash scripts/verify_e2e.sh hiereb
...
STREAM_TIME_MODE="${STREAM_TIME_MODE:-$(dotenv_or_default STREAM_TIME_MODE wall_clock)}"
...
AUTO_DETECT_HOUSE_IDS="${AUTO_DETECT_HOUSE_IDS:-$(dotenv_or_default AUTO_DETECT_HOUSE_IDS 1)}"
AUTO_DETECT_TIME_WINDOW="${AUTO_DETECT_TIME_WINDOW:-$(dotenv_or_default AUTO_DETECT_TIME_WINDOW 0)}"
...
WARMUP_START="${WARMUP_START:-$(dotenv_or_default WARMUP_START 1377993600)}"
WARMUP_END="${WARMUP_END:-$(dotenv_or_default WARMUP_END 1378598399)}"
EVAL_START="${EVAL_START:-$(dotenv_or_default EVAL_START 1378598400)}"
EVAL_END="${EVAL_END:-$(dotenv_or_default EVAL_END 1379203199)}"
EPSILON_H="${EPSILON_H:-$(dotenv_or_default EPSILON_H 0.05)}"
EPSILON_RATIO="${EPSILON_RATIO:-$(dotenv_or_default EPSILON_RATIO "$EPSILON_H")}"
...
services:
  simulator:
    environment:
      ...
      STREAM_TIME_MODE: "$STREAM_TIME_MODE"
      HOUSE_IDS: '$HOUSE_IDS_JSON'
      AUTO_DETECT_HOUSE_IDS: "$AUTO_DETECT_HOUSE_IDS"
      AUTO_DETECT_TIME_WINDOW: "$AUTO_DETECT_TIME_WINDOW"
